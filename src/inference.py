...
def naive_topK(matrix, K, axis=0):
...
def load_ground_truth(ground_truth_path, subset):
    ...
    class_labels_map = get_class_labels(data)  # Category name(str) <-> Category number(int)
...
def calculate_clip_acc(clips_results, ground_truth, class_labels_map):
    ...
    for (video_id, true_label) in ground_truth:
        ...
        for per_clip_result in video_result_list:
            predict_label = class_labels_map[per_clip_result['result'][0]['label']]  # class name -> class number
...
class Inference:
    ...
    def __call__(self, predict_data, model, annotation_path):
        ...
        for step, data in enumerate(predict_data.create_dict_iterator(output_numpy=True)):
            ...
            video_ids, segments = zip(*total_target_data['targets'][str(label[0])])
            ...
            sorted_score, locs = topk_(outputs, K=1)
            locs = locs.reshape(1, -1)

            t2 = time.time()
            print("[{} / {}] Net time : {} ms".format(step, step_size, (t2 - t1) * 1000))
            # print("[{}/{}] Net time : {} ms\n[video_id] : {}\n[Predict] : {}".format(
            #     step, step_size, (t2 - t1) * 1000, video_ids[0], locs
            # ))
